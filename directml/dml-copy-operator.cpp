    ...
    const std::vector<Dml::D3D12BufferRegion>& output_buffer_regions)
{
    ...
    input_uav_desc.Buffer.NumElements = static_cast<uint32_t>(input_buffer_regions[0].SizeInBytes() / m_input_dtype_size);
    ...
    output_uav_desc.Buffer.NumElements = static_cast<uint32_t>(output_buffer_regions[0].SizeInBytes() / m_output_dtype_size);
