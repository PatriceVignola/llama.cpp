...
namespace Dml
{
    ...
    std::unique_ptr<size_t> PooledUploadHeap::FindOffsetForAllocation(const Chunk& chunk, size_t sizeInBytes)
    {
        ...
        if (firstAllocation.offsetInChunk <= lastAllocation.offsetInChunk)
        {
            ...
            if (newAllocationBegin + sizeInBytes <= chunk.capacityInBytes)
            {
                ...
                return std::make_unique<size_t>(newAllocationBegin);
            }
            else
            {
    ...
    std::pair<PooledUploadHeap::Chunk*, size_t> PooledUploadHeap::Reserve(size_t sizeInBytes)
    {
        ...
        {
            auto offsetForAllocation = FindOffsetForAllocation(chunk, sizeInBytes);
        ...
        size_t newChunkSize = std::max({ m_totalCapacity, c_minChunkSize, sizeInBytes });

        while (newChunkSize > UINT32_MAX) {
            Trim();
            newChunkSize = std::max({ m_totalCapacity, c_minChunkSize, sizeInBytes });
        }
        ...
        uint64_t size)
    {
        ...
        std::tie(chunk, offsetInChunk) = Reserve(size);

        assert(chunk != nullptr);
        assert(offsetInChunk + size <= chunk->capacityInBytes);
        ...
        chunk->allocations.push_back(Allocation{ static_cast<size_t>(size), offsetInChunk, doneEvent });
