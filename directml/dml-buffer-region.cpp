...
namespace Dml
{
    ...
    {
        THROW_HR_IF(E_INVALIDARG, m_resource == nullptr);

        // Regions cannot be empty.
        THROW_HR_IF(E_INVALIDARG, m_sizeInBytes == 0);

        // Regions cannot extend beyond the size of the resource.
        uint64_t bufferSize = m_resource->GetDesc().Width;
        THROW_HR_IF(E_INVALIDARG, m_offset >= bufferSize);
        THROW_HR_IF(E_INVALIDARG, m_sizeInBytes > bufferSize - offset);
