...
namespace Dml
{
    ...
    {
        DescriptorRange(ID3D12DescriptorHeap* heap, D3D12_CPU_DESCRIPTOR_HANDLE cpuHandle, D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle)
            : heap(heap)
            , cpuHandle(cpuHandle)
            , gpuHandle(gpuHandle) {}
    ...
    public:
        ...
        std::unique_ptr<DescriptorRange> TryAllocDescriptors(
