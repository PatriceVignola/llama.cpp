...
namespace Dml
{
    ...
    void ExecutionContext::FillBufferWithPattern(ID3D12Resource* dstBuffer, uint64_t offset, const uint8_t* data, uint64_t size)
    {
        SetCommandRecorder(&m_dmlRecorder);
        m_dmlRecorder.FillBufferWithPattern(dstBuffer, offset, data, size);
    }
    ...
    void ExecutionContext::SetCommandRecorder(DmlCommandRecorder* newRecorder)
    {
        assert(!m_closed);
